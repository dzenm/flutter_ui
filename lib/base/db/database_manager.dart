...
class DatabaseManager {
  ...
  static const String _PRAGMA_TABLE = 'PRAGMA table_info';
  static const String _SELECT_ALL_TABLE = "SELECT * FROM sqlite_master WHERE TYPE='table'";
  ...
  Future<Database> getDatabase({String? dbName}) async {
    if (_database == null) {
      ...
      _database = await openDatabase(databasesPath,
  ...
  Future<void> _onUpgrade_1_2(Database db, int oldVersion,
  ...
  Future<List<TableEntity>> getTableList({String? dbName}) async {
    Database db = await getDatabase(dbName: dbName);
    List<dynamic> list = await db.rawQuery(_SELECT_ALL_TABLE);
    Log.d('查询所有表: $list');
  ...
  Future<void> checkTable(String tableName, String columnString) async {
    bool isTableExist = await this.isTableExist(tableName);
    if (!isTableExist) {
  ...
  Future<void> _createTable(Database db, String tableName, String columnString) async {
    String sql = '$_CREATE_TABLE $tableName ($columnString)';
  ...
  Future<List<ColumnEntity>> getTableColumn(String dbName, String tableName) async {
    Database db = await getDatabase(dbName: dbName);
    List list = await db.rawQuery('$_PRAGMA_TABLE($tableName)');
    Log.d('查询表列名: $list');
  ...
  Future<List<Map<String, dynamic>>> where(
    ...
    if (where == null) {
      ...
      Log.d('表$tableName查询 ${paramsString.toString()} 数据${list.length}条: $list',
          tag: _TAG);
    }
  ...
  void _handleMap(Map<String, String> whereMap, List<String> whereArgs,
      StringBuffer where, StringBuffer paramsString) {
