...
class _RankPageState extends State<RankPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('积分排行榜', style: TextStyle(color: Colors.white)),
  ...
  Widget _buildCollectItem(CoinEntity coin, int index) {
    String title = coin.username ?? '';
    return TapLayout(
      child: ListTile(
        title: Text(title),
      ),
  ...
  void _getCoins({bool isReset = false}) {
    ...
    Future.delayed(Duration(milliseconds: isReset ? 500 : 0), () {
      ...
      HttpManager.instance.getRankCoinList(
        page: _page,
        isShowDialog: false,
