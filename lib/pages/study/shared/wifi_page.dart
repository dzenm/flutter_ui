...
class _FlutterWifiIoTState extends State<FlutterWifiIoT> {
  ...
  Widget getWidgets() {
    ...
    if (_htResultNetwork != null && _htResultNetwork!.length > 0) {
      ...
      return SingleChildScrollView(
        child: SafeArea(
          child: Column(
            children: Platform.isIOS ? getButtonWidgetsForiOS() : getButtonWidgetsForAndroid(),
          ),
        ),
      );
    }
    ...
    if (_isWifiAPSupported) {
      htPrimaryWidgets.addAll(<Widget>[
        ...
        FutureBuilder(
            ...
            builder: (BuildContext context, AsyncSnapshot<WIFI_AP_STATE?> wifiState) {
      ...
      FutureBuilder(
          ...
          builder: (BuildContext context, AsyncSnapshot<List<String>> info) {
            htPrimaryWidgets.addAll(<Widget>[
              ...
              MaterialButton(
                color: Colors.blue,
                child: Text("Set AP info ($AP_DEFAULT_SSID/$AP_DEFAULT_PASSWORD)", style: textStyle),
                onPressed: () {
      ...
      htPrimaryWidgets.add(const Center(child: Text("Wifi AP probably not supported by your device")));
    }
  ...
  Widget build(BuildContext poContext) {
    return MaterialApp(
      ...
      home: Scaffold(
        appBar: AppBar(
          title: Platform.isIOS ? const Text('WifiFlutter Example iOS') : const Text('WifiFlutter Example Android'),
                    ...
                    onSelected: (value) {
                      switch (value) {
                        case "disconnect":
                          ...
                          break;
                        case "remove":
                          WiFiForIoTPlugin.getSSID().then((val) => WiFiForIoTPlugin.removeWifiNetwork(val!));
                          break;
                        default:
                          break;
                      }
