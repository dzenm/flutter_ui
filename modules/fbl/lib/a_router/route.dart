...
/// The page builder for [ARoute].
typedef ARouterPageBuilder = Page<dynamic> Function(
  BuildContext context,
  ARouterState state,
);

/// The widget builder for [ARoute].
typedef ARouterWidgetBuilder = Widget Function(
  BuildContext context,
  ARouterState state,
);
...
/// The base class for [ARoute] and [ShellRoute].
///
...
///
/// To create sub-routes for a route, provide them as a [ARoute] list
/// with the sub routes.
///
...
class ARoute extends RouteBase {
  /// Constructs a [ARoute].
  ...
  })  : assert(path.isNotEmpty, 'ARoute path cannot be empty'),
        assert(name == null || name.isNotEmpty, 'ARoute name cannot be empty'),
  ...
  /// Whether this [ARoute] only redirects to another route.
  ///
  ...
  ///
  /// This is used in [ARouter.namedLocation] and its related API. This
  ...
  /// `/family/456` and etc. The parameter values are stored in [ARouterState]
  ...
  final ARouterPageBuilder? pageBuilder;
  ...
  /// ```
  ///
  final ARouterWidgetBuilder? builder;

  /// Called when this route is removed from ARouter's route history.
  ///
  /// Some example this callback may be called:
  ///  * This route is removed as the result of [ARouter.pop].
  ///  * This route is no longer in the route history after a [ARouter.go].
  ///
...
abstract class ShellRouteBase extends RouteBase {
  ...
  static void _debugCheckSubRouteParentNavigatorKeys(
    ...
    for (final RouteBase route in subRoutes) {
      ...
      if (route is ARoute && route.redirectOnly) {
...
class ShellRouteContext {
  ...
  final ARouterState routerState;
...
///
/// When a ShellRoute is added to the list of routes on ARouter or ARoute, a
...
/// [parentNavigatorKey] that matches the key provided to either the [ARouter]
...
class ShellRoute extends ShellRouteBase {
  ...
  ///
  /// Similar to [ARoute.builder], but with an additional child parameter. This
  ...
  ///
  /// Similar to [ARoute.pageBuilder], but with an additional child parameter.
  ...
  /// All ShellRoutes build a Navigator by default. Child ARoutes
...
class StatefulShellRoute extends ShellRouteBase {
  ...
  ///
  /// Similar to [ARoute.builder], but with an additional
  ...
  ///
  /// Similar to [ARoute.pageBuilder], but with an additional
  /// [StatefulNavigationShell] parameter. StatefulNavigationShell is a Widget
  /// responsible for managing the nested navigation for the
  /// matching sub-routes. Typically, a shell route builds its shell around this
  /// Widget. StatefulNavigationShell can also be used to access information
...
class StatefulShellBranch {
  ...
  ///
  /// If none is specified, the location of the first descendant [ARoute] will
  ...
  ARoute? get defaultRoute =>
      RouteBase.routesRecursively(routes).whereType<ARoute>().firstOrNull;
}
...
class StatefulNavigationShell extends StatefulWidget {
  ...
  String _effectiveInitialBranchLocation(int index) {
    ...
    if (initialLocation != null) {
      ...
      /// find the first ARoute, from which a full path will be derived.
      final ARoute route = branch.defaultRoute!;
