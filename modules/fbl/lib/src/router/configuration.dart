...
typedef ARouterRedirect = FutureOr<String?> Function(
    BuildContext context, ARouterState state);

/// The route configuration for ARouter configured by the app.
class RouteConfiguration {
      ...
      if (route is ARoute) {
        ...
        if (parentKey != null) {
          ...
          assert(
              ...
              " an ancestor ShellRoute's navigatorKey or ARouter's"
              ' navigatorKey');
  ...
  static bool _debugVerifyNoDuplicatePathParameter(
      List<RouteBase> routes, Map<String, ARoute> usedPathParams) {
    for (final RouteBase route in routes) {
      if (route is! ARoute) {
        continue;
      }
      ...
      if (route is StatefulShellRoute) {
        for (final StatefulShellBranch branch in route.branches) {
          if (branch.initialLocation == null) {
            // Recursively search for the first ARoute descendant. Will
            ...
            assert(
                ...
                'derivable from ARoute descendant');
            assert(
  ...
  void _onRoutingTableChanged() {
    ...
    assert(_debugVerifyNoDuplicatePathParameter(
        routingTable.routes, <String, ARoute>{}));
  ...
  /// Builds a [ARouterState] suitable for top level callback such as
  /// `ARouter.redirect` or `ARouter.onException`.
  ARouterState buildTopLevelARouterState(RouteMatchList matchList) {
    return ARouterState(
      this,
      uri: matchList.uri,
  ...
  /// The list of top level routes used by [ARouterDelegate].
  List<RouteBase> get routes => _routingConfig.value.routes;

  /// Top level page redirect.
  ARouterRedirect get topRedirect => _routingConfig.value.redirect;
  ...
  ///
  /// When navigating using [ARouter.go] or [ARouter.push], one can provide
  ...
  /// Looks up the url location by a [ARoute]'s name.
  ...
  RouteMatchList findMatch(Uri uri, {Object? extra}) {
    ...
    if (matches.isEmpty) {
      return _errorRouteMatchList(
        uri,
        AException('no routes for location: $uri'),
        extra: extra,
      );
    }
    return RouteMatchList(
  ...
  FutureOr<RouteMatchList> redirect(
    ...
    FutureOr<RouteMatchList> processRedirect(RouteMatchList prevMatchList) {
      ...
      final FutureOr<String?> topRedirectResult = _routingConfig.value.redirect(
        context,
        buildTopLevelARouterState(prevMatchList),
      );
  ...
  void log(String message) {
    if (_routingConfig.value.debugLog == null) return;
    _routingConfig.value.debugLog!(message, tag: 'ARouter');
  }
}
