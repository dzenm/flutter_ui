...
abstract class RouteMatchBase with Diagnosticable {
  ...
  static Map<GlobalKey<NavigatorState>?, List<RouteMatchBase>>
    ...
    } else if (route is ARoute) {
      result = _matchByNavigatorKeyForARoute(
        route: route,
        matchedPath: matchedPath,
        remainingLocation: remainingLocation,
        matchedLocation: matchedLocation,
...
/// An matched result by matching a [ARoute] against a location.
///
...
/// The route match that represent route pushed through [ARouter.push].
class ImperativeRouteMatch extends RouteMatch {
  ...
  static ARoute _getsLastRouteFromMatches(RouteMatchList matchList) {
  ...
  /// The completer for the future returned by [ARouter.push].
...
///
/// This corresponds to the ARouter's history.
@immutable
class RouteMatchList with Diagnosticable {
  ...
  /// ```
  ///
  /// The [matches] must be the in same order of how ARoutes are matched.
  ///
  /// ```dart
    ...
    if (configuration.extraCodec != null) {
      ...
      try {
        ...
        configuration.log(
            ...
            'without a codec. Consider provide a codec to ARouter to '
