...
import 'observer_widget_scope.dart';
import 'observer_widget_tag_manager.dart';
...
class ObserverWidget<C extends ObserverController, M extends ObserveModel,
  ...
  static ObserverWidgetState<C, M, N, T>? maybeOf<
    ...
    if (tag != null) {
      ...
      myCtx = tagManager?.context(tag);
    }
    return (myCtx ?? context)
...
class ObserverWidgetState<
  ...
  void _handleScopeContext(BuildContext myCtx) async {
    scopeContext = myCtx;
    final tag = widget.tag ?? '';
    if (tag.isEmpty) return;
    await WidgetsBinding.instance.endOfFrame;
    assert(myCtx.mounted);
    final tagManager = ObserverWidgetTagManager.maybeOf(myCtx);
    tagManager?.set(tag, myCtx);
  }
  ...
  void removeListener({
    ...
    final myListeners = innerListeners;
    if (myListeners == null) return;
    for (final ObserverListenerEntry<M> entry in myListeners) {
  ...
  void _notifyListeners(
    ...
    final myListeners = innerListeners;
    if (myListeners == null || myListeners.isEmpty) return;

    final List<ObserverListenerEntry<M>> localListeners =
        List<ObserverListenerEntry<M>>.of(myListeners);
      ...
      try {
        if (entry.list != null) {
          ...
          if (entry.onObserve != null) {
            ...
            BuildContext? myContext = entry.context;
            if (myContext == null && targetSliverContexts.isNotEmpty) {
              myContext = targetSliverContexts.first;
            }
            final result = changeResultMap[myContext];
